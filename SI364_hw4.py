...
#app.config["SQLALCHEMY_DATABASE_URI"] = "postgresql://user:Wisdom729@localhost5000/durantkmHW4"
app.config["SQLALCHEMY_DATABASE_URI"] = "postgresql://postgres:Wisdom729@localhost:5432/durantkmHW4"
...
class User(db.Model):
    ...
    #def __repr__(self):
    #    return "{} by {} | {}".format(self.twitter_username)
# - Hashtag
## -- id (Primary Key)
## -- text (Unique=True)
class Hashtag(db.Model):
    ...
    #tweets= db.relationship('Tweet', secondary=Tweet_Hashtag,backref = db.backref('hashtag', lazy ='dynamic'),lazy = 'dynamic')
    #def __repr__(self):
    #    return "{} by {} | {}".format(self.text)
...
class TweetForm(FlaskForm):
    text = StringField("What is the tweet you wish existed?", validators =[Required()])
    username = StringField("What is the tweeter's username?", validators =[Required()])
    hashtags = StringField("Give us one or more hashtags(separated by commas)", validators=[Required()])
...
def get_or_create_user(db_session,user_name):
    username = db_session.query(User).filter_by(twitter_username = user_name).first()
    if username:
        return username
    else:
        username = User(twitter_username=user_name)
...
def get_or_create_hashtag(db_session, hashtag_text):
    ...
    if hashtag:
        ...
        db_session.commit()
        return(hashtag)
def get_or_create_tweet(db_session,tweet_user, tweet_text, hashtag_list=[]):
    tweet = db_session.query(Tweet).filter_by(text=tweet_text).first()
    if tweet:
        return(tweet)
    else:
        User = get_or_create_user(db_session, tweet_user)
        tweet = Tweet(user_id = User.id, text = tweet_text)
...
# get_or_create_tweet(db_session,tweet_user, tweet_text, hashtag_list=[]):
    ...
    if form.validate_on_submit():
        ...
        get_or_create_tweet(db.session, form.username.data, form.text.data, form.hashtags.data.split(","))
...
def see_all_tweets():
    ...
    tweets = Tweet.query.all()
    for tweet in tweets:
        user_search = User.query.filter_by(id=tweet.user_id).first()
        all_tweets.append((tweet.text,user_search.twitter_username))
...
def see_all_users():
    ...
    usernames = [(username.twitter_username,len(Tweet.query.filter_by(user_id = username.id).all())) for username in users]
